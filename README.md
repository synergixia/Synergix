...
| `IMAGE_GEN_MODE` | `serverless` | Image backend: `serverless` (RunPod) or `http` (pod / local CPU) |
| `RUNPOD_ENDPOINT_ID` | _(empty)_ | RunPod Serverless endpoint id (serverless mode) |
| `RUNPOD_API_KEY` | _(empty)_ | RunPod API key, sent as Bearer (serverless mode) |
...
Images are generated by an SDXL worker (`image-gen-gpu/handler.py`) running on
**RunPod Serverless** — it scales to zero, so you only pay per second of
generation. The bot calls the RunPod API; auth is your RunPod API key (Bearer).

1. **Build & push the worker image:**
   ```bash
   docker build -f image-gen-gpu/Dockerfile.serverless \
     -t <your-dockerhub-user>/synergix-image-gen-serverless image-gen-gpu
   docker push <your-dockerhub-user>/synergix-image-gen-serverless
   ```
2. **Create a Serverless endpoint** on RunPod from that image:
   - Pick a GPU (e.g. RTX 2000 Ada, 16 GB).
   - Attach a **network volume** (mounted at `/runpod-volume`) so the SDXL
     weights, cached under `HF_HOME=/runpod-volume/hf-cache`, persist between
     cold starts.
   - Note the **Endpoint ID**.
3. **Wire the bot** — in the host `.env`:
   ```
   IMAGE_GEN_MODE=serverless
   RUNPOD_ENDPOINT_ID=<your endpoint id>
   RUNPOD_API_KEY=<your RunPod API key>
   ```
   then `docker compose up -d bot`.

Quick check (replace id/key):
```bash
curl -s https://api.runpod.ai/v2/<endpoint_id>/health \
  -H "Authorization: Bearer <RUNPOD_API_KEY>"
```

> **Alternatives** (set `IMAGE_GEN_MODE=http`): a persistent RunPod **pod**
> running `image-gen-gpu/app.py` (set `IMAGE_GEN_HOST` to its proxy URL +
> `IMAGE_GEN_API_KEY`), or the **local CPU fallback** `image-gen/` (SD 1.5),
> started with `docker compose --profile local-image up -d image-gen` and
> `IMAGE_GEN_HOST=http://image-gen:8084`.
