...
| `IMAGE_GEN_HOST` | `http://image-gen:8084` | Image generator endpoint (set to the RunPod proxy URL) |
| `IMAGE_GEN_API_KEY` | _(empty)_ | Shared `X-API-Key` for the remote generator (must match the pod) |
...
### Image generation (RunPod GPU)

Images are generated by a small diffusers/SDXL service (`image-gen-gpu/`) that
runs on a RunPod GPU pod, not on the main host. The bot calls it over HTTP and
authenticates with a shared `X-API-Key`.

1. **Build & push the image** (from a machine with Docker + a registry login):
   ```bash
   docker build -t <your-dockerhub-user>/synergix-image-gen-gpu image-gen-gpu
   docker push <your-dockerhub-user>/synergix-image-gen-gpu
   ```
2. **Create a RunPod GPU pod** (e.g. RTX 2000 Ada, 16 GB) from that image:
   - Expose **HTTP port 8084**.
   - Set env `IMAGE_GEN_API_KEY` to a long random secret (`openssl rand -hex 32`).
   - Optionally attach a network volume mounted at `/workspace` so the SDXL
     weights (downloaded from Hugging Face on first run) persist across restarts.
3. **Wire the bot** — in the host `.env`:
   ```
   IMAGE_GEN_HOST=https://<podid>-8084.proxy.runpod.net
   IMAGE_GEN_API_KEY=<the same secret>
   ```
   then `docker compose up -d bot`.

Quick check (replace URL/key):
```bash
curl -s -o /dev/null -w "%{http_code}\n" https://<podid>-8084.proxy.runpod.net/health   # 200 = ready
```

> **Local CPU fallback:** the `image-gen/` service (stable-diffusion.cpp, SD 1.5)
> still exists for hosts without a GPU. Start it with
> `docker compose --profile local-image up -d image-gen` and set
> `IMAGE_GEN_HOST=http://image-gen:8084` (no API key needed on the private network).
